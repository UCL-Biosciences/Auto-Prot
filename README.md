...
Create the environment and download dependencies using the information in configuration files: `conda create -f .../configs/auto-prot-env-YOUR-OS-HERE.yml`. Which version you use will depend on whether you are using a Windows machine or a Mac.
...
### Run the pipeline
To generate an html report, we need a new conda environment. First deactivate the active environment with `conda deactivate`. Then make the environment for converting the template to an html report: `conda create -f .../configs/auto-prot-env-markdown-YOUR-OS-HERE.yml`. Activate it `conda activate markdown`.
