...
### 4. Generate outputs
#### Input
The repository contains example protein and metadata files: `proteindata.csv` and `metadata.csv` files, based on training data from Foll (2021). This will give a quick picture of how the input data should be formatted. You can overwrite these files with your own data (requirements given in `/docs/Workflow.md`). 

Full reference for the training data: Melanie Christine Föll. Training data for MaxQuant and Msstats label-free analysis in Galaxy. Zenodo; 2021. Recreated under CC4.0 licence. Downloaded and processed in `r_scripts/DE-with-MSstats-Limma.R`.
