...
This project uses a standard Python package layout and is installable via `pip`. The core code lives in the `autoprot/` directory and is defined as a package using `pyproject.toml`. Note: The package is not published on PyPI. Users must clone the repository and install it locally.

To work on the project: `pip install -e .`. This installs the package in editable mode, meaning any code changes you make will be reflected immediately without needing a rebuild.
...
To see the test coverage, first `pip install pytest-cov`, then `pytest --cov=autoprot --cov-report=term-missing` from the main folder. This will show coverage for the autoprot package and highlight any lines not exercised by tests.
