...
col_names = list(
    map(lambda x: "prot_abundance_" + str(x), sample_replicates)
)  # col names have a string before as we expect real data will
...
cols_for_set_1 = list(
    map(
        lambda x: "prot_abundance_" + str(x),
        sample_replicates[:n_samples_per_treatment],
    )
)
cols_for_set_2 = list(
    map(
...
metadata_df = pd.DataFrame(
    {
        "sample_id": [
            item for item in samples for _ in range(n_replicates)
        ],  # sample ID is each sample (for item in samples) as many times as there are replicates ( for _ in range(n_replicates) )
        "replicate": list(range(1, n_replicates + 1))
        * len(samples),  # for each sample, replicate will be 1:n_replicates
...
print("Generated protein data and metadata and saved to file. All done.")
