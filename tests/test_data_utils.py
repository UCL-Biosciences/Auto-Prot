...
import pandas as pd
import pytest
from utils.data_utils import validate_metadata, validate_proteindata
...
def test_duplicate_sample_ids_case_insensitive():
    ...
    meta = pd.DataFrame({"sample_rep": ["S1", "s1"], "protein_abundance_name": ["S1", "s1"]})
...
def test_duplicate_protein_ids_case_insensitive():
    df = pd.DataFrame(
        {"s1": [1], "s2": [2]},
        index=["GeneA", "genea"]  # duplicate ignoring case
    )
    meta = pd.DataFrame({"sample_rep": ["s1", "s2"], "protein_abundance_name": ["s1", "s2"]})
